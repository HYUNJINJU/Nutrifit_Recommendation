...
# 데이터 로드
file_path = '/content/drive/MyDrive/전국통합식품영양성분정보_음식_표준데이터.csv'
...
# 데이터 정규화
scaler = MinMaxScaler()
numeric_columns = selected_columns[1:-1]  # 영양소 컬럼만 정규화
nutrition_data_grouped[numeric_columns] = scaler.fit_transform(nutrition_data_grouped[numeric_columns])
...
# 사용자 데이터 로드
with open('/content/drive/MyDrive/user_data.json', 'r', encoding='utf-8') as file:
...
# json 파일에서 섭취 데이터 로드
with open('/content/drive/MyDrive/food_data.json', 'r', encoding='utf-8') as file:
...
# Autoencoder 학습
autoencoder, encoder_model = build_autoencoder(len(numeric_columns))
autoencoder.fit(
    nutrition_data_grouped[numeric_columns].values,
    nutrition_data_grouped[numeric_columns].values,
    epochs=10,
    batch_size=8,
    verbose=1
...
def recommend_foods(deficiencies, nutrition_data, category1, category2, category3, taste1, taste2):
    ...
    def recommend_random_from_top(df, n_top=3):
        """
        상위 n_top개의 음식 중에서 무작위로 하나 선택.
        """
        top_n_df = df.head(n_top)
        if len(top_n_df) > 1:
            return top_n_df.sample(n=1, random_state=random.randint(1, 100))
        return top_n_df
    ...
    if not primary_data.empty:
        ...
        top_primary_recommendation = primary_data.sort_values(by="추천점수", ascending=False)

        # 상위 5개 중 무작위 선택
        top_primary_recommendation = recommend_random_from_top(primary_data, n_top=5)
    ...
    if is_rice:
        for category in category2:
            ...
            if not second_data.empty:
                ...
                # 상위 5개 중 무작위 선택
                top_second_recommendation = recommend_random_from_top(second_data, n_top=5)
                recommendations = pd.concat([recommendations, top_second_recommendation], ignore_index=True)
                break


    # category3, category4, category5에서 각각 하나씩 추천 (taste2 반영)
    ...
    for category_list in [category3, category4, category5]:
        for category in category_list:
            ...
            if not third_data.empty:
                ...
                # 상위 5개 중 무작위 선택
                top_third_recommendation = recommend_random_from_top(third_data, n_top=5)
...
# 결과 출력 전에 정규화된 값을 원래 값으로 복원하고 반올림
def restore_original_values(recommendations, scaler, numeric_columns):
    # 정규화된 값을 복원
    original_values = scaler.inverse_transform(recommendations[numeric_columns])
    # 복원된 값을 소수점 첫째 자리까지 반올림
    recommendations[numeric_columns] = np.round(original_values, 1)
    # 추천점수를 소수점 둘째 자리까지 반올림
    recommendations["추천점수"] = np.round(recommendations["추천점수"], 2)
    return recommendations

# 복원된 값 출력
recommended_foods_original = restore_original_values(recommended_foods_specific.copy(), scaler, numeric_columns)

# 결과 출력
print("\n추천된 음식과 영양성분:")
print(tabulate(
    recommended_foods_original[["대표식품명", "추천점수"] + selected_columns[1:]],
    headers="keys",
    tablefmt="pretty"))

# 추천 결과를 json 형식으로 저장
output_file = 'recommended_foods.json'
recommended_foods_json = recommended_foods_original[["대표식품명", "추천점수"] + selected_columns[1:]].to_dict(orient='records')
